...
function searchNSEW () {
    ...
    if (cpuLastHitRow < 0 && !(isAttackingTwice(list))) {
        return 1
    }
    grid.place(cursor, grid.add(tiles.getTileLocation(cpuLastHitCol, cpuLastHitRow), 0, 1))
    if (cpuLastHitRow < 6 && !(isAttackingTwice(list))) {
        return 1
    }
    grid.place(cursor, grid.add(tiles.getTileLocation(cpuLastHitCol, cpuLastHitRow), 1, 0))
    if (cpuLastHitRow < 9 && !(isAttackingTwice(list))) {
        return 1
    }
    grid.place(cursor, grid.add(tiles.getTileLocation(cpuLastHitCol, cpuLastHitRow), -1, 0))
    if (cpuLastHitCol < 9 && !(isAttackingTwice(list))) {
        return 1
    }
    return 0
}
